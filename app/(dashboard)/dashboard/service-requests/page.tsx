...
export default function ServiceRequestsPage() {
  ...
  const assignMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["service-request", selectedId] });
    },
    onError: (error) => toast.error(getApiErrorMessage(error)),
  });

  const statusMutation = useMutation({
    mutationFn: (nextStatus: string) => api.updateServiceRequestStatus(selectedId!, nextStatus),
    onSuccess: () => {
  ...
  return (
    <div>
          ...
          <>
            <div className="overflow-x-auto rounded-[24px] border border-[#343434]">
              <table className="min-w-full">
                <thead className="bg-[#2a2a2a] text-left text-[16px] font-medium text-white">
                  <tr>
                    {["Date & Time", "User Name", "Technician's Name", "Service", "Status", "Activity", "Action"].map(
                      (heading) => (
                ...
                <tbody>
                  {listQuery.data?.requests.map((request) => (
                    <tr className="border-t border-[#343434]" key={request._id}>
                      ...
                      <td className="px-6 py-5 text-[#d4d4d4]">{request.serviceType}</td>
                      <td className="px-6 py-5">
                        <StatusBadge value={request.paymentStatus === "paid" ? "Paid" : "Non paid"} />
                      </td>
                      <td className="px-6 py-5">
                        <StatusBadge value={request.status.replaceAll("_", " ")} />
                      </td>
                      <td className="px-6 py-5">
                        <button onClick={() => setSelectedId(request._id)} type="button">
            ...
            <div className="mt-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <p className="text-[#9b9b9b]">
                Showing {(page - 1) * 10 + 1} to {(page - 1) * 10 + (listQuery.data?.requests.length ?? 0)} of {listQuery.data?.total ?? 0} results
              </p>
              <Pagination
                page={page}
      ...
      <Modal open={!!selectedId} onClose={() => setSelectedId(null)} className="max-w-[1512px]">
        {detail ? (
              ...
              <div>
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <Textarea value={detail.presentingProblem || detail.problemDescription} readOnly />
                  </div>
                  <div>
                    <label className="mb-3 block text-[18px] font-semibold">Diagnosis</label>
                    <Textarea value={detail.diagnosis || "No diagnosis provided"} readOnly />
                  </div>
                  <div>
                    <label className="mb-3 block text-[18px] font-semibold">Post Service Documentation</label>
                    <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                      {(detail.postServicePhotos?.length ? detail.postServicePhotos : detail.evidencePhotos).map((photo, index) => (
                        <div
                  ...
                  {detail.additionalCosts.length ? (
                    <div className="border-t border-[#d4a100] pt-8">
                      <h3 className="mb-5 text-[18px] font-semibold">Additional Charges</h3>
                      {detail.additionalCosts.map((cost) => (
                        <div className="rounded-2xl bg-[#6a6a6a] p-5" key={cost._id}>
                  ...
                  <Button className="h-16 w-full text-[18px]" onClick={() => setSelectedId(null)}>
                    Done
                  </Button>
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div className="rounded-[22px] bg-[#6a6a6a] p-6">
                  ...
                  <div className="mt-6 flex items-center gap-4">
                    ...
                    />
                    <div>
                      <p className="text-[18px] text-[#ffc737]">{detail.assignedTechnician?.name}</p>
                      <p className="text-sm">
                        ★ {detail.assignedTechnician?.rating?.average ?? 0} ({detail.assignedTechnician?.rating?.count ?? 0} reviews)
                      </p>
                    </div>
                  </div>
                </div>
                        ...
                        <span>
                          {formatCurrency(
                            (detail.serviceFee ?? 0) +
                              ...
                              detail.additionalCosts.reduce((sum, cost) => sum + cost.amount, 0),
                          )}
                        </span>
                      </div>
                    </div>
            ...
            <div>
              <div className="mb-6 flex flex-wrap items-center justify-between gap-4 rounded-[24px] border border-[#4d4d4d] p-5">
                <div className="flex items-center gap-4">
                  <Avatar className="size-16" name={detail.customer.name} src={detail.customer.avatar} />
                  <div>
                    <p className="text-[20px] font-semibold">{detail.customer.name}</p>
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  <Button onClick={() => statusMutation.mutate("confirmed")}>Confirm</Button>
                </div>
              </div>
              ...
              <div className="grid gap-6 xl:grid-cols-[1.2fr_1fr]">
                <div className="rounded-[24px] border border-[#4d4d4d] p-6">
                  <div className="grid gap-6 md:grid-cols-2">
                    <div>
                      <label className="mb-3 block text-[18px]">Technician</label>
                      <Select
                        onChange={(event) =>
                      ...
                      >
                        ...
                        {technicians.map((technician) => (
                          <option key={technician._id} value={technician._id}>
                            {technician.name} - {technician.serviceName || technician.service}
                          </option>
                        ))}
                      </Select>
                    </div>
                  ...
                  <div className="mt-6 rounded-[20px] border border-[#4d4d4d] p-4">
                    <h3 className="text-[18px] font-semibold">Evidence & Context</h3>
                    ...
                    <div className="mt-6 grid gap-6 md:grid-cols-3">
                      <div>
                        <label className="mb-3 block text-[18px]">Special Instruction</label>
                        <Input value={detail.specialInstruction || "No special instructions"} readOnly />
                      </div>
                      <div>
                        <label className="mb-3 block text-[18px]">Service Area</label>
                        <div className="relative">
                          <Input
                            value={detail.address.address || detail.address.street || "Location unavailable"}
                            readOnly
                          />
                      ...
                      <div>
                        <label className="mb-3 block text-[18px]">Service Type</label>
                  ...
                  <div className="mt-6">
                    <h3 className="mb-4 text-[20px] font-semibold">Fix Booking Schedule for User</h3>
                    <div className="rounded-[20px] bg-[#505050] p-4">
                      <p className="mb-5 text-[18px]">Service Schedule</p>
                      <div className="grid gap-6 md:grid-cols-2">
                        <div>
                          <label className="mb-3 block text-[18px] text-[#f4c542]">Preferred Date</label>
                          <div className="relative">
                            <Input
                        ...
                        <div>
                          <label className="mb-3 block text-[18px] text-[#f4c542]">Preferred Time</label>
                          <div className="relative">
                            <Input
                              className="border-transparent bg-[#696969]"
                              onChange={(event) =>
                  ...
                  <div className="mt-6 rounded-[20px] bg-[#505050] p-4">
                    <div className="grid gap-6 md:grid-cols-2">
                      <div>
                        <label className="mb-3 block text-[18px]">Service Fee</label>
                        <Input
                          className="border-transparent bg-[#696969]"
                          onChange={(event) =>
                            setAssignForm((current) => ({
                      ...
                      <div>
                        <label className="mb-3 block text-[18px]">Facility Tax</label>
                        <Input
                          className="border-transparent bg-[#696969]"
                          onChange={(event) =>
                            setAssignForm((current) => ({
                ...
                <div className="rounded-[24px] border border-[#4d4d4d] p-6">
                  <h3 className="mb-5 text-[20px] font-semibold">Assignment Summary</h3>
                  <div className="space-y-4 rounded-[20px] bg-[#2f2f2f] p-5">
                    <p className="text-[18px]">
                      Customer: <span className="text-[#f4c542]">{detail.customer.name}</span>
                    </p>
                    <p className="text-[18px]">
                      Technician:{" "}
                      <span className="text-[#f4c542]">
                        {technicians.find((technician) => technician._id === assignForm.technicianId)?.name || "Select one"}
                      </span>
                    </p>
                    <p className="text-[18px]">Service Fee: {formatCurrency(assignForm.serviceFee)}</p>
                    <p className="text-[18px]">Facility Tax: {formatCurrency(assignForm.facilityTax)}</p>
