...
import { Eye, MessageSquareText, Search, Trash2 } from "lucide-react";
          ...
          <>
            <div className="overflow-x-auto rounded-[24px] border border-[#343434]">
              <table className="min-w-full">
                <thead className="bg-[#2a2a2a] text-left text-[16px] font-medium text-white">
                  <tr>
                    {["User Name", "Email", "Service", "Status", "Action"].map(
                      (heading) => (
                ...
                <tbody>
                  {listQuery.data?.technicians.map((technician) => (
                    <tr
                      className="border-t border-[#343434]"
                      key={technician._id}
                    >
                      ...
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-4 text-white">
                          ...
                          <button
                            onClick={() => setSelectedId(technician._id)}
                            type="button"
                          >
            ...
            <div className="mt-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <p className="text-[#9b9b9b]">
                ...
                {(page - 1) * 10 + (listQuery.data?.technicians.length ?? 0)} of{" "}
          ...
          <div>
            <div className="mb-8 flex justify-end gap-3">
              <Button
                onClick={() =>
                  statusMutation.mutate({
                    id: detailQuery.data.technician._id,
                    isActive: !detailQuery.data.technician.isActive,
                  })
                }
                variant={
                  detailQuery.data.technician.isActive ? "danger" : "primary"
                }
              >
                <Trash2 className="mr-2 size-4" />
                {detailQuery.data.technician.isActive
                  ? "Deactivate"
                  : "Activate"}
              </Button>
            ...
            <div className="text-center">
              ...
              <p className="mt-5 text-[30px] font-medium">
                {detailQuery.data.technician.name}
              </p>
            </div>
            <div className="mt-8 grid gap-4 rounded-[22px] bg-[#4b4b4b] p-6 md:grid-cols-3">
              <div>
                <p className="text-sm text-[#d9d9d9]">Status</p>
                <div className="mt-3">
                  <StatusBadge
                    value={
                      detailQuery.data.technician.isActive
              ...
              <div>
                <p className="text-sm text-[#d9d9d9]">Availability</p>
                <p className="mt-3 text-[20px] font-semibold text-[#f4c542]">
                  {detailQuery.data.technician.availability || "offline"}
                </p>
              </div>
              <div>
            ...
            <div className="mt-8 grid gap-6 md:grid-cols-3">
              <div>
                ...
                <p>
                  {detailQuery.data.technician.serviceName ||
                    detailQuery.data.technician.service}
                </p>
              </div>
              <div>
